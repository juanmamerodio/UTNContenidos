...
const GAS_API_URL = "https://script.google.com/macros/s/AKfycbxW6AAtWvfHxwUXS9b0P6mZioizAEsqRCrtTx00JTvbygI12BH0zcxC4CKiY4vasSU6/exec"
...
// ---------------------------------------------------------------------------
// OBTENCIÓN DINÁMICA DE IMÁGENES Y WIDGETS DE SLIDES v4.0
// ---------------------------------------------------------------------------
function fetchSlideImage(keyword, index = 0, topicName = '') {
    const kw = (keyword || topicName || 'engineering technology').trim();
    const cleanKw = encodeURIComponent(kw.replace(/[^a-zA-Z0-9\s]/g, ''));
    return `https://image.pollinations.ai/prompt/professional%20hd%20engineering%20photo%20${cleanKw}?width=800&height=450&nologo=true&seed=${index + 100}`;
}

window.handleSlideImgError = function(img) {
    img.onerror = null;
    img.src = 'https://images.unsplash.com/photo-1581091226825-a6a2a5aee158?auto=format&fit=crop&w=800&q=80';
};

function renderVisualWidget(widget) {
    if (!widget || !widget.widgetType) return '';
    if (widget.widgetType === 'kpi' && widget.valorDestacado) {
        return `
            <div class="ui-widget-kpi">
                <div class="kpi-value">${sanitizeHTML(widget.valorDestacado)}</div>
                <div class="kpi-label">${sanitizeHTML(widget.etiqueta || 'Métrica Clave')}</div>
            </div>
        `;
    }
    if (widget.widgetType === 'timeline' && widget.pasos && widget.pasos.length > 0) {
        return `
            <div class="ui-widget-timeline">
                ${widget.pasos.map((paso, i) => `
                    <div class="timeline-step-item">
                        <span class="timeline-step-num">${i + 1}</span>
                        <span>${sanitizeHTML(paso)}</span>
                    </div>
                `).join('')}
            </div>
        `;
    }
    if (widget.widgetType === 'callout') {
        return `
            <div class="ui-widget-callout">
                <strong>💡 ${sanitizeHTML(widget.etiqueta || 'Reto / Punto Clave')}</strong>
                <p>${sanitizeHTML(widget.valorDestacado || 'Debate en aula')}</p>
            </div>
        `;
    }
    if (widget.widgetType === 'badge_list' && widget.itemsBadge && widget.itemsBadge.length > 0) {
        return `
            <div class="ui-widget-badge-list">
                ${widget.itemsBadge.map(item => `<span class="ui-badge-pill">📌 ${sanitizeHTML(item)}</span>`).join('')}
            </div>
        `;
    }
    return '';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function ejecutarGeneracionIA(materiaNombre, temaNombre, contextoDinamico, linkTeoria) {
        ...
        try {
            ...
            if (respuesta && respuesta.success) {
                ...
                // C. Estructura de Diapositivas Sugerida (Diseño Universitario de Alto Impacto v4.0)
                ...
                if (slidesGrid && respuesta.slides) {
                    slidesGrid.innerHTML = respuesta.slides.map((slide, index) => {
                        ...
                        // Generar o resolver la URL de imagen contextual
                        const imgUrl = fetchSlideImage(slide.imagenKeyword, index, breadcrumbTopic.textContent);
                        const imgHTML = `
                            <div class="slide-image-container">
                                <img src="${imgUrl}" alt="${sanitizeHTML(slide.titulo)}" onerror="handleSlideImgError(this)" loading="lazy">
                                <span class="slide-image-tag">${sanitizeHTML(slide.imagenKeyword || 'Ilustración Cátedra')}</span>
                            </div>
                        `;

                        // Renderizar Widget Visual
                        const widgetHTML = renderVisualWidget(slide.visualWidget);
    ...
    // --- 7c. REPRODUCTOR DE SLIDES A PANTALLA COMPLETA (WEB DECK PRESENTER v4.0) ---
    const btnPresentFullscreen = document.getElementById('btn-present-fullscreen');
    const modalSlidesPlayer = document.getElementById('modal-slides-player');
    ...
    const playerSlideStage = document.getElementById('player-slide-stage');
    const playerNotesPanel = document.getElementById('player-notes-panel');
    const playerNotesText = document.getElementById('player-notes-text');

    let playerSlideIndex = 0;

    function renderPlayerSlide(index) {
        if (!claseGeneradaActual || !claseGeneradaActual.slides || claseGeneradaActual.slides.length === 0) return;
        const total = claseGeneradaActual.slides.length;
        if (index < 0) index = 0;
        if (index >= total) index = total - 1;
        playerSlideIndex = index;

        const slide = claseGeneradaActual.slides[playerSlideIndex];
        if (playerCounter) playerCounter.textContent = `Slide ${playerSlideIndex + 1} / ${total}`;
        if (playerTitle) playerTitle.textContent = `${slide.categoria ? '[' + slide.categoria.toUpperCase() + '] ' : ''}${slide.titulo}`;

        const imgUrl = fetchSlideImage(slide.imagenKeyword, playerSlideIndex, breadcrumbTopic.textContent);
        const widgetHTML = renderVisualWidget(slide.visualWidget);

        let stageHTML = '';
        if (slide.tipo === 'portada') {
            stageHTML = `
                <div class="player-slide-canvas hero-bg">
                    <div>
                        <span style="background: rgba(255,255,255,0.15); color: #93c5fd; padding: 6px 14px; border-radius: 20px; font-size: 0.85rem; font-weight: 700; text-transform: uppercase;">
                            UTN Facultad Regional Delta
                        </span>
                        <h1 style="font-size: 2.5rem; font-weight: 800; margin: 20px 0 10px 0; line-height: 1.2; color: #ffffff;">
                            ${sanitizeHTML(slide.titulo)}
                        </h1>
                        <p style="font-size: 1.25rem; color: #cbd5e1; font-weight: 500; margin-bottom: 24px;">
                            ${sanitizeHTML(slide.subtitulo || breadcrumbSubject.textContent || '')}
                        </p>
                    </div>
                    <div style="display: flex; gap: 24px; align-items: center;">
                        <div style="flex: 1; height: 220px; border-radius: 12px; overflow: hidden; border: 1px solid rgba(255,255,255,0.2);">
                            <img src="${imgUrl}" alt="Portada" style="width:100%; height:100%; object-fit:cover;" onerror="handleSlideImgError(this)">
                        </div>
                        <div style="flex: 1;">
                            ${widgetHTML}
                        </div>
            ...
            stageHTML = `
                <div class="player-slide-canvas">
                    <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid rgba(255,255,255,0.1); padding-bottom: 16px; margin-bottom: 24px;">
                        <div>
                            <span style="font-size: 0.8rem; color: #60a5fa; font-weight: 700; text-transform: uppercase;">${sanitizeHTML(slide.categoria || 'Tema')}</span>
                            <h2 style="font-size: 1.9rem; font-weight: 700; color: #ffffff; margin: 4px 0 0 0;">${sanitizeHTML(slide.titulo)}</h2>
                        </div>
                        <span style="font-size: 0.9rem; color: #64748b; font-weight: 600;">Cátedra UTN FRD</span>
                    </div>

                    <div style="display: grid; grid-template-columns: ${slide.imagenKeyword ? '1fr 340px' : '1fr'}; gap: 32px; align-items: center; flex: 1;">
                        <div>
                            ${widgetHTML}
                            <ul style="list-style: none; padding: 0; margin: 16px 0; display: flex; flex-direction: column; gap: 14px;">
                                ${lineas.map(line => `
                                    <li style="font-size: 1.15rem; line-height: 1.5; color: #e2e8f0; display: flex; align-items: flex-start; gap: 10px;">
                                        <span style="color: #60a5fa; font-weight: bold;">•</span>
                                        <span>${sanitizeHTML(line.replace(/^[•\-\*]\s*/, ''))}</span>
                                    </li>
                                `).join('')}
                            </ul>
                        </div>
                        ${slide.imagenKeyword ? `
                            <div style="height: 260px; border-radius: 12px; overflow: hidden; border: 1px solid rgba(255,255,255,0.15); box-shadow: 0 10px 25px rgba(0,0,0,0.4);">
                                <img src="${imgUrl}" alt="${sanitizeHTML(slide.titulo)}" style="width: 100%; height: 100%; object-fit: cover;" onerror="handleSlideImgError(this)">
                            </div>
                        ` : ''}
                    </div>
                </div>
        ...
        if (playerSlideStage) playerSlideStage.innerHTML = stageHTML;
        if (playerNotesText) playerNotesText.textContent = slide.notasOrador || 'Sin notas de aula para esta diapositiva.';
    }

    if (btnPresentFullscreen) {
        btnPresentFullscreen.addEventListener('click', () => {
            if (!claseGeneratedCheck()) return;
            playerSlideIndex = 0;
            renderPlayerSlide(0);
            if (modalSlidesPlayer) modalSlidesPlayer.showModal();
        });
    }

    if (btnPlayerNext) btnPlayerNext.addEventListener('click', () => renderPlayerSlide(playerSlideIndex + 1));
    if (btnPlayerPrev) btnPlayerPrev.addEventListener('click', () => renderPlayerSlide(playerSlideIndex - 1));
    if (btnClosePlayer) btnClosePlayer.addEventListener('click', () => modalSlidesPlayer && modalSlidesPlayer.close());

    if (btnToggleNotes) {
        btnToggleNotes.addEventListener('click', () => {
            if (playerNotesPanel) {
                const isHidden = playerNotesPanel.style.display === 'none' || !playerNotesPanel.style.display;
                playerNotesPanel.style.display = isHidden ? 'flex' : 'none';
            }
        });
    }

    // Atajos de teclado para navegación de diapositivas
    document.addEventListener('keydown', (e) => {
        if (modalSlidesPlayer && modalSlidesPlayer.open) {
            if (e.key === 'ArrowRight' || e.key === ' ' || e.key === 'PageDown') {
                e.preventDefault();
                renderPlayerSlide(playerSlideIndex + 1);
    ...
    [modalLoader, modalSuccess, modalContexto, modalSlidesPlayer].forEach(modal => {
        if (!modal) return;
        modal.addEventListener('click', (e) => {
            const dialogDimensions = modal.getBoundingClientRect();
            if (e.clientX < dialogDimensions.left || e.clientX > dialogDimensions.right || e.clientY < dialogDimensions.top || e.clientY > dialogDimensions.bottom) {
                if (modal.id === 'modal-success' || modal.id === 'modal-contexto' || modal.id === 'modal-slides-player') modal.close();
            }
        });
    });
